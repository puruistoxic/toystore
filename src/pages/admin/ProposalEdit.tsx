...
export default function ProposalEdit() {
  ...
  const { data: proposal, isLoading } = useQuery<Proposal>({
    ...
    queryFn: async () => {
      const response = await invoicingApi.getProposal(id!);
      console.log('[Proposal Edit] API Response:', response.data);
      if (response.data) {
        console.log('[Proposal Edit] Proposal from API:', {
          id: response.data.id,
          client_id: response.data.client_id,
          client_name: response.data.client_name,
          client_address: response.data.client_address,
          client_city: response.data.client_city,
          client_state: response.data.client_state,
          hasAddress: !!response.data.client_address,
          hasCity: !!response.data.client_city,
          hasState: !!response.data.client_state
        });
      }
  ...
  const handleDownloadPDF = async () => {
    if (!proposal) return;
    try {
      console.log('=== Downloading PDF from Edit Page ===');
      console.log('Proposal from edit page:', proposal);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => invoicingApi.updateProposal(id!, data),
    onSuccess: () => {
  ...
  if (isLoading) {
    return (
      <AdminLayout title="Edit Proposal">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Edit Proposal</h2>
              <p className="text-sm text-gray-600 mt-1">Update proposal details</p>
            </div>
          </div>
          {proposal && (
